...
const TenantEdit = (props: any) => {
    ...
    return (
        <TasksContext.Provider value={value}>
            ...
            >
                <DialogContent>
                    <Edit {...props} resource="tenants" id={props.id}>
                        {/* <TenantEditDialog id={id}/> */}
                    </Edit>
                </DialogContent>
            </Dialog>
...
const SimpleFormToolBar = (props: any) => {
    ...
    const handleClose = () => {
        ...
        dispatch({
            type: 'setActualTenant',
            payload: {actualTenant: record},
        });
    ...
    const handleApply = () => {
        ...
        if (state.actualTenant) {
            const actualTenant = state.actualTenant;
            const diff = { componentsIds: actualTenant.componentsIds };
...
const TenantEditDialog = ({ id }: { id?: string }) => {
...
export const ComponentsListEdit = (props: any) => {
    ...
    if (!state.actualTenant) return null;
    const actualTenant = state.actualTenant;
    ...
    const { data: components, isLoading: isLoadingRecordComponents } = useGetMany<Component>(
        'components',
        { ids: actualTenant.componentsIds },
        { enabled: actualTenant && actualTenant.componentsIds && state.actualTenant.componentsIds.length > 0 }
    );

    const unselectedComponents =
    allComponents && allComponents.filter(component => !actualTenant.componentsIds.includes(component.id));

    const handleDeleteComponent = (id: number) => {
        const components = actualTenant.componentsIds.filter((componentId: number) => componentId !== id);
        actualTenant.componentsIds = components
        dispatch({
            type: 'setActualTenant',
            payload: {actualTenant: actualTenant},
        });
        const comp1 = !record.componentsIds.filter( c => !actualTenant.componentsIds.includes(c) ).length;
        const comp2 = !actualTenant.componentsIds.filter(c => !record.componentsIds.includes(c)).length;
    ...
    const handleAddComponent = (id: number) => {
        if (! actualTenant.componentsIds.includes(id)) {
            const components = [...actualTenant.componentsIds, id];
            actualTenant.componentsIds = components;
            dispatch({
                type: 'setActualTenant',
                payload: {actualTenant: actualTenant},
